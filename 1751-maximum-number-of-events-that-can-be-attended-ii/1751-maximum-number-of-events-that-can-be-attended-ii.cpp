    ...
    int BS(int index,vector<vector<int>>&events,int value)
{
    ...
    int fn(int i,vector<vector<int>>&events,int k, vector<vector<int>>&dp)
{
  ...
  int c1 = events[i][2] + fn(index,events,k-1,dp);
  int c2 = fn(i+1,events,k,dp);
